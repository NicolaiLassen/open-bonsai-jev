...
# bonzi-8b-ternary-v1-jev

**Ternary Bonsai 1 8B as a typed decision function.** No weights here. This is a
recipe and a measurement for running
[`prism-ml/Ternary-Bonsai-8B-gguf`](https://huggingface.co/prism-ml/Ternary-Bonsai-8B-gguf) as a
...
```json
{"choice": "true", "probabilities": {"true": 0.999912, "false": 8.8e-05},
 "label_mass": 0.999991, "prompt_tokens": 93, "forward_s": 0.177}
```
...
| | |
|---|---|
| Weights | [`Ternary-Bonsai-8B-PQ2_0.gguf`](https://huggingface.co/prism-ml/Ternary-Bonsai-8B-gguf/blob/main/Ternary-Bonsai-8B-PQ2_0.gguf) |
| Quantization | ternary (`PQ2_0`) |
| On disk | **2.18 GB** |
| Runtime | PrismML fork |
| WANLI-256 | **65.2%** (chance 33.3%) |
| easy100 | 100% |
| Throughput | 200 decisions/min |
| Median latency | 281 ms |
| Median `label_mass` | 0.999 |
...
Apple M4 Pro, GPU, ctx 4096, warmup excluded. This model ranks **#3 of
...
cd open-bonzi-jev
./scripts/build_llama_fork.sh
python3 scripts/download_model.py Ternary-Bonsai-8B

./openjev/score.sh -q "Is the user angry?" \
    --state '{"msg":"this is broken AGAIN"}' \
    --model bonzi/models/Ternary-Bonsai-8B
```
