...
**Bonsai 1 27B as a typed decision function.** No weights here. This is a
recipe and a measurement for running
[`prism-ml/Bonsai-27B-gguf`](https://huggingface.co/prism-ml/Bonsai-27B-gguf) as a
...
```json
{"choice": "true", "probabilities": {"true": 0.99925, "false": 0.00075},
 "label_mass": 0.992323, "prompt_tokens": 95, "forward_s": 1.161}
```
...
| | |
|---|---|
| Weights | [`Bonsai-27B-Q1_0.gguf`](https://huggingface.co/prism-ml/Bonsai-27B-gguf/blob/main/Bonsai-27B-Q1_0.gguf) |
| Quantization | 1-bit (`Q1_0`) |
| On disk | **3.80 GB** |
| Runtime | stock llama.cpp |
| WANLI-256 | **71.1%** (chance 33.3%) |
| easy100 | 100% |
| Throughput | 31 decisions/min |
| Median latency | 1911 ms |
| Median `label_mass` | 0.949 |
...
Apple M4 Pro, GPU, ctx 4096, warmup excluded. This model ranks **#2 of
...
python3 scripts/download_model.py Bonsai-27B

./openjev/score.sh -q "Is the user angry?" \
    --state '{"msg":"this is broken AGAIN"}' \
    --model bonzi/models/Bonsai-27B
```
